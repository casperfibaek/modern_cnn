...
from .utils import get_activation, get_normalization, CoordAttBlock


class CoreCNNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, *, norm="batch", activation="relu", padding="same", residual=True, use_attention=True):
        ...
        self.conv3 = nn.Conv2d(self.out_channels, self.out_channels, 1, padding=0, groups=1)
    ...
    def __init__(self, depth, in_channels, out_channels, norm="batch", activation="relu", padding="same"):
        ...
        for i in range(self.depth):
            ...
            block = CoreCNNBlock(
                ...
                norm=self.norm, activation=self.activation, padding=self.padding,
                use_attention=(i == self.depth - 1),
            )
...
class CoreDecoderBlock(nn.Module):
    def __init__(self, depth, in_channels, out_channels, *, norm="batch", activation="relu", padding="same"):
        ...
        self.match_channels = CoreCNNBlock(self.in_channels * 2, self.out_channels, norm=self.norm, activation=self.activation_blocks, padding=self.padding, use_attention=False)
        ...
        for i in range(self.depth):
            block = CoreCNNBlock(
                ...
                norm=self.norm, activation=self.activation_blocks, padding=self.padding,
                use_attention=(i == self.depth - 1),
            )
            self.blocks.append(block)
...
class CoreEncoder(nn.Module):
    def __init__(self, *,
        ...
        self.stem = CoreCNNBlock(self.in_chans, self.dims[0], norm=self.norm, activation=self.activation, padding=self.padding)
        ...
        self.head = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(self.dims[-1], self.num_classes),
        )
